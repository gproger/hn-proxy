    ...
    while st_c != res_len:
        if res[st_c] == '>':
            ...
            if st_c != st_s:
                ...
                #  (?<!/|-) remove matching preceeding word of 6 elements by / or - symbol
                #  this symbols may use in link or - word annotations
                #  \\b[a-zA-Z]{6}\\b - start word (\\b) word with only 6 symbols and (\\b) end world
                #  (?!<) - test for after world no < symbol - start next tag used for correct "parent" matching
